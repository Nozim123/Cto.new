...
import { Building2, Construction, Mail, Map, MapPin, Phone, Sparkles, Store, Users } from 'lucide-react'
...
export default function HomePage() {
  ...
  const stats = [
    { label: t('home.activeMalls'), value: mallsData.filter((m) => m.status === 'open').length, icon: Building2 },
    {
      label: t('home.totalStores'),
      value: mallsData
        .filter((m) => m.status === 'open')
        .reduce((acc, m) => acc + (Number(m.storeCount) || 0), 0),
      suffix: '+',
      icon: Store
    },
    { label: t('home.comingSoon'), value: mallsData.filter((m) => m.status === 'coming_soon').length, icon: Construction },
  ...
  const contacts = [
    ...
    { icon: Mail, title: 'Email Us', info: 'roziyevnozim43@gmail.com', color: 'from-green-400 to-green-600' },
    ...
    <div className="min-h-screen overflow-hidden pb-20 md:pb-0">
      ...
      <section className="relative z-10 px-4 py-8 md:hidden">
        <div className="max-w-md mx-auto">
          <h3 className="text-lg font-bold text-white mb-4 text-center">{t('home.explore')}</h3>
          <div className="grid grid-cols-2 gap-3">
            <button
              className="py-3 px-4 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors text-sm flex items-center justify-center"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 mt-8 md:mt-12">
              <div className="group animate-slide-in-left">
                <div className="p-4 md:p-6 rounded-xl md:rounded-2xl bg-white/10 backdrop-blur-lg border border-white/10 hover:bg-white/20 transition-all duration-500 transform hover:-translate-y-2 hover:shadow-purple-glow">
                  ...
                  <h3 className="text-lg md:text-xl font-semibold mb-2 md:mb-3 text-white">{t('home.discover')}</h3>
              ...
              <div className="group animate-slide-in-right" style={{ animationDelay: '0.2s' }}>
                <div className="p-4 md:p-6 rounded-xl md:rounded-2xl bg-white/10 backdrop-blur-lg border border-white/10 hover:bg-white/20 transition-all duration-500 transform hover:-translate-y-2 hover:shadow-purple-glow">
                  ...
                  <h3 className="text-lg md:text-xl font-semibold mb-2 md:mb-3 text-white">{t('home.experiences')}</h3>
                  <p className="text-sm md:text-base text-gray-300 leading-relaxed">
      ...
      <section id="malls" className="relative z-10 py-12 md:py-20">
        ...
        <div className="mt-8 md:mt-16 grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
          {stats.map((stat, index) => (
            ...
            >
              <div className="text-xl md:text-3xl mb-2 flex justify-center">
                <stat.icon size={28} className="text-purple-200" />
              </div>
      ...
      <section className="relative z-10 py-16 md:py-32">
        <div className="relative max-w-6xl mx-auto text-center px-4">
          <div className="animate-fade-in-up">
            <h2 className="text-3xl sm:text-4xl lg:text-6xl font-bold text-white mb-4 md:mb-6">{t('home.virtual')}</h2>
            ...
            <div className="flex flex-col sm:flex-row gap-3 md:gap-6 justify-center items-center">
              <button className="bg-white text-purple-700 px-6 py-3 rounded-lg hover:bg-purple-50 transition-colors font-semibold w-full sm:w-auto">
                {t('buttons.startVirtualTour')}
              </button>

              <button className="border-2 border-white/30 text-white px-6 py-3 rounded-lg hover:bg-white/10 backdrop-blur-sm transition-colors font-semibold w-full sm:w-auto flex items-center justify-center">
      ...
      <section id="contact" className="relative z-10 py-12 md:py-20">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto animate-fade-in-up">
            ...
            <p className="text-sm md:text-lg text-gray-300 mb-6 md:mb-12 leading-relaxed">{t('home.questionsDescription')}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
              {contacts.map((contact, index) => (
                ...
                >
                  <div
                    className={`w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-gradient-to-r ${contact.color} flex items-center justify-center mx-auto mb-3 md:mb-4 group-hover:scale-110 transition-transform`}
                  >
                    <contact.icon className="text-white" size={22} />
                  </div>
                  <h3 className="text-sm md:text-lg font-semibold text-white mb-2">{contact.title}</h3>
                  <p className="text-xs md:text-sm text-gray-300 break-words">{contact.info}</p>
                </div>
              ))}
            </div>
          </div>
