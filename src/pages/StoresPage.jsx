      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 pt-10 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <div className="rounded-2xl border border-gray-200 bg-white dark:border-white/10 dark:bg-white/5 p-4">
            <label className="text-xs font-semibold text-gray-600 dark:text-white/70">{t('search.search') || 'Search'}</label>
            <input
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder={t('stores.searchPlaceholder') || 'Search stores…'}
              className="mt-2 w-full bg-transparent border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 outline-none text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-white/50"
            />
          </div>
          <div className="rounded-2xl border border-gray-200 bg-white dark:border-white/10 dark:bg-white/5 p-4">
            <label className="text-xs font-semibold text-gray-600 dark:text-white/70">{t('stores.categories') || 'Categories'}</label>
            <select
              value={category}
              onChange={(e) => setCategory(e.target.value)}
              className="mt-2 w-full bg-transparent border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 outline-none text-gray-900 dark:text-white"
            >
              {categories.map((c) => (
        ...
        {filtered.length > 0 ? (
          ...
          <div className="text-center py-20 rounded-3xl border border-gray-200 bg-white dark:border-white/10 dark:bg-white/5">
            ...
            <p className={darkMode ? 'text-white/70' : 'text-gray-600'}>{t('search.tryDifferent') || 'Try a different keyword.'}</p>
          </div>
        )}
      </div>
    </div>
