      ...
      >
        ...
        <div className="grid md:grid-cols-2 gap-6 p-6 md:p-8">
                ...
                {images.map((img, idx) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-20 h-20 rounded-lg overflow-hidden border-2 transition-all duration-300 ${
                      selectedImage === idx ? 'border-purple-500 scale-105' : 'border-gray-300 hover:border-purple-300'
                    }`}
                  >
          ...
          <div className="flex flex-col">
            {/* Category */}
            <div className="text-xs font-bold text-purple-600 uppercase tracking-wider mb-2">{product.category}</div>
            ...
            <div className="flex items-baseline gap-3 mb-6">
              <span className={`text-3xl md:text-4xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                ${product.price.toFixed(2)}
              </span>
              {product.tag && <span className="text-lg text-gray-400 line-through">${(product.price * 1.2).toFixed(2)}</span>}
            </div>

            {/* Availability */}
            <div className="flex items-center gap-2 mb-6">
              <div className={`w-2 h-2 rounded-full ${product.availability === 'In Stock' ? 'bg-green-500' : 'bg-red-500'}`}></div>
              <span
                ...
                <div className="space-y-2">
                  {Object.entries(product.specifications).map(([key, value]) => (
                    <div key={key} className="flex justify-between text-sm">
                      ...
                      <span className={darkMode ? 'text-gray-200' : 'text-gray-900'}>{value}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="mt-auto space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Button3D variant="outline" className="py-2 flex items-center justify-center gap-2">
                  <Heart size={16} />
                  {t('buttons.addToWishlist') || 'Save'}
                </Button3D>
                <Button3D variant="outline" className="py-2 flex items-center justify-center gap-2">
                  <Share2 size={16} />
